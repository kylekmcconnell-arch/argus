...
`HELIUS_API_KEY`) add depth. Treat `COINGECKO_API_KEY` as required wherever real
scans run: the keyless CoinGecko tier throttles after a few registry queries, and a
throttled query leaves token identity unavailable. See `.env.example` for the full
