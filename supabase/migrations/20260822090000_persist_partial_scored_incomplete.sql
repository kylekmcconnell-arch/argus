-- Let an honest INCOMPLETE report carry the PARTIAL axis scores the engine
-- actually produced. Since P0, finalize() publishes every axis it could score
-- on an incomplete role report (the preliminary-score surface renders them),
-- and argus_is_routing_failed_report already distinguishes partial-scored
-- INCOMPLETE reports from decisionless ones. The lineage gate still required
-- zero scored axes on any INCOMPLETE report, so every partial-scored scan
-- failed certification and no such report could persist or activate. Scored
-- axes on an incomplete report now certify under exactly the complete-report
-- axis rules (bounded refs, eligibility, absence gaps, exact link matching);
-- only the minimum-count floors move: a role report may score zero axes and
-- the report zero axes overall ONLY while the composite verdict is INCOMPLETE
-- with a withheld governing score.
...
begin
  ...
  if exists (
    ...
    ) not between (case when v_is_incomplete then 0 else 1 end) and 80
  ) then
  ...
  from pg_catalog.jsonb_array_elements(v_role_reports) role_report(item)
  cross join lateral pg_catalog.jsonb_each(role_report.item -> 'axes') axis(axis_id, item);
  if (not v_is_incomplete and v_scored_axis_count < 1)
     or v_scored_axis_count > 1280 then
  ...
  if v_link_count < v_scored_axis_count or v_link_count > 1024 then
...
comment on function public.enforce_axis_evidence_lineage() is
  'Certifies strict immutable report lineage; INCOMPLETE reports may carry partial scored axes under the same per-axis rules.';
