-- Forward-only repair for PostgreSQL 42702 from the bundle RPC.
-- The RETURNS TABLE output named report_version_id is also a PL/pgSQL
-- variable, so bare report_version_id conflict targets are ambiguous when
-- each INSERT is first planned. Named constraints make the database-column
-- intent explicit without changing the immutable replay behavior.
