...
- The reported 31-token turnover cohort has only 13 token and eight wallet records. Its intent is now unestablished; funding/template observations remain indexed without inventing the missing population or common control.
...
The other 18 reported cohort tokens, full wallet population, verified manipulation evidence, Catalyst escrow/payout identities and custody analysis have not been established. Complete launch histories, reliable cross-chain rankings, Arkham/Fomo enrichment, historical revenue/CEX/stock-pair data and validated behavioral alerts remain the broader issue #525 work. This patch does not claim to complete those data programs or to implement swap-aware creator conduct.
