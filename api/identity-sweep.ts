...
async function handleHistory(handle: string): Promise<{ prior: string[]; firstSeen: string | null }> {
  const d = await getJson(`https://api.memory.lol/v1/tw/${encodeURIComponent(handle)}`);
...
async function github(u: string, key?: string): Promise<Hit | null> {
  const d = await getJson(`https://api.github.com/users/${encodeURIComponent(u)}`, key ? { authorization: `Bearer ${key}`, accept: "application/vnd.github+json" } : undefined);
...
async function farcaster(u: string): Promise<Hit | null> {
  const d = await getJson(`https://api.warpcast.com/v2/user-by-username?username=${encodeURIComponent(u)}`);
...
async function reddit(u: string): Promise<Hit | null> {
  const d = await getJson(`https://www.reddit.com/user/${encodeURIComponent(u)}/about.json`);
  const data = d?.data;
...
async function telegram(u: string): Promise<Hit | null> {
  const html = await getText(`https://t.me/${encodeURIComponent(u)}`);
  if (!html) return null;
...
async function footprintFor(u: string, key?: string): Promise<Hit[]> {
  const [g, f, r, t] = await Promise.all([github(u, key), farcaster(u), reddit(u), telegram(u)]);
...
async function archivedBio(handle: string): Promise<{ handle: string; year: string; bio: string } | null> {
  const avail = await getJson(`https://archive.org/wayback/available?url=twitter.com/${encodeURIComponent(handle)}`);
  const snap = avail?.archived_snapshots?.closest;
  if (!snap?.available || !snap.timestamp) return null;
  const html = await getText(`https://web.archive.org/web/${snap.timestamp}id_/https://twitter.com/${handle}`);
  if (!html) return null;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { prior, firstSeen } = await handleHistory(handle);
    ...
    const [footprints, priorBios] = await Promise.all([
      Promise.all(usernames.map((u) => footprintFor(u, key).then((hits) => ({ username: u, hits })))),
      Promise.all(prior.slice(0, 3).map((h) => archivedBio(h))),
    ]);
