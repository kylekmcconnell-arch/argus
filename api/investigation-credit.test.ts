...
describe("POST /api/investigation-credit", () => {
  ...
  it("returns the charged and remaining visible credits", async () => {
    ...
    await handler({ method: "POST", body: { idempotencyKey: "scan-key-123", kind: "token" } } as never, res);
    expect(captured).toEqual({ status: 200, body: { allowed: true, chargedCredits: 1, remainingCredits: 49_999 } });
  ...
  it("returns an explicit reason and balance when credits are exhausted", async () => {
    ...
    const { res, captured } = response();
    await handler({ method: "POST", body: { idempotencyKey: "scan-key-123", kind: "investigation" } } as never, res);
