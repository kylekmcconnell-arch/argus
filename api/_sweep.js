...
async function resolveEvmCreatorKind(chain, creator, fetchImpl = fetch) {
  const origin = globalThis.location?.origin;
  if (!origin) return "unknown";
  try {
    const r = await fetchImpl(`/api/bytecode?address=${encodeURIComponent(creator)}&chain=${encodeURIComponent(chain)}`, { signal: AbortSignal.timeout(12e3) });
