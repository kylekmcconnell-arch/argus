...
interface Prov { key: string; also?: string; label: string; powers: string; source: string; tier: "paid" | "optional" | "infra"; live?: "github" }
...
const PROVIDERS: Prov[] = [
  ...
  { key: "HELIUS_API_KEY", label: "Helius (Solana)", powers: "Deployer, funding trails, mint history, serial-launch sweep", source: "dashboard.helius.dev", tier: "paid" },
...
export default async function handler(_req: VercelRequest, res: VercelResponse) {
  ...
  const providers = PROVIDERS.map((p) => {
    const configured = !!process.env[p.key] && (!p.also || !!process.env[p.also]);
    return {
      label: p.label,
      powers: p.powers,
      source: p.source,
