  ...
  try {
    ...
    for (const { txHash, amount } of dedupTx) {
      ...
      if (BURN_ADDRS.has(ownerLc)) {
        positions.push({ tokenId, owner, ownerKind: "burned", ownerName: null, amount: amount.toString(), locked: true, reason: "The position NFT sits at a burn address — the liquidity can never be withdrawn by anyone." });
        continue;
      }
      ...
      if (typeof code === "string") {
        ...
        positions.push({
          ...
          reason: found.length
            ? `Held by an unverified contract (${owner}) whose bytecode exposes ${found.map((s) => ESCAPE_SELECTORS[s].split("(")[0]).join("/")} — it can move the position.`
            : `Held by an unverified contract (${owner}) — no source to confirm a lock; its bytecode exposes no standard NFT-transfer selector, but a custom exit path can't be ruled out from bytecode alone.`,
        });
      }
    }
