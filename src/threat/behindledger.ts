...
// Single fetch, no retry: the endpoint is expensive (a budgeted 40s chain walk)
...
export async function behindLedger(chain: string, address: string, pairAddress?: string | null): Promise<BehindLedgerReport | null> {
  try {
    ...
    const res = await apiFetch(`/api/behindledger?address=${encodeURIComponent(address)}&chain=${encodeURIComponent(chain)}${pair}`, {
