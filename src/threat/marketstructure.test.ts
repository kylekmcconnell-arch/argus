...
const mk = (high: number, low: number, close: number, volumeUsd: number): Candle => ({ ts: (t0 += DAY), high, low, close, volumeUsd });
...
function cycle(): Candle[] {
  ...
  for (let i = 0; i < 30; i++) ks.push(mk(1.05, 0.95, 1.04, 100));
  // markup to $2 on thin volume
  for (let i = 0; i < 10; i++) {
    const p = 1.05 + (i + 1) * 0.09;
    ks.push(mk(p + 0.01, p - 0.05, p, 20));
  }
  // top near $2: heavier volume still, closes pinned to the lows (sellers unloading)
  for (let i = 0; i < 30; i++) ks.push(mk(2.05, 1.95, 1.96, 150));
  return ks;
}
...
describe("analyzeMarketStructure", () => {
  it("returns null on thin, flat, or volumeless data", () => {
    ...
    expect(analyzeMarketStructure(Array.from({ length: 8 }, () => mk(1.1, 0.9, 1, 50)))).toBeNull();
    // flat price: no span to bin
    expect(analyzeMarketStructure(Array.from({ length: 20 }, () => mk(1, 1, 1, 50)))).toBeNull();
    // no volume anywhere: unmeasured is not zero, but there is nothing to profile
    expect(analyzeMarketStructure(Array.from({ length: 20 }, (_, i) => ({ ts: t0 + (i + 1) * DAY, high: 1.02 + i * 0.01, low: 0.99 + i * 0.01, close: 1.01 + i * 0.01 })))).toBeNull();
  });
  ...
  it("profile conserves volume and concentration lands where trade happened", () => {
    ...
    const total = ks.reduce((s, k) => s + (k.volumeUsd ?? 0), 0);
  ...
  it("fib levels follow the swing direction", () => {
    ...
    const down = analyzeMarketStructure(cycle().reverse().map((k, i) => ({ ...k, ts: 1_700_000_000 + i * DAY })))!;
  ...
  it("volume trend: rising when the tail outpaces the prior window, with up/down split", () => {
    ...
    // recent closes never rise above the previous close: no recent volume on up bars
    expect(ms.volume.recentUpShare).toBe(0);

    // dying tape: same shape but the tail volume collapses
    const dying = cycle().map((k, i, arr) => (i >= arr.length - 14 ? { ...k, volumeUsd: 5 } : k));
  ...
  it("mid-range closes with balanced flow read as consolidation", () => {
    const ks: Candle[] = [];
    for (let i = 0; i < 40; i++) ks.push(mk(1.1, 0.9, 1.0, 100)); // closes dead-center
    for (let i = 0; i < 5; i++) ks.push(mk(1.3, 1.0, 1.25, 10)); // small spike widens the window
    const ms = analyzeMarketStructure(ks)!;
  ...
  it("flags fib confluence when a level lands inside a range", () => {
    ...
    const ks: Candle[] = [];
    ks.push(mk(1.0, 0.0, 0.5, 1)); // pins swing low 0, part of window
    ks.push(mk(2.0, 1.0, 1.9, 1)); // pins swing high 2
    for (let i = 0; i < 30; i++) ks.push(mk(1.05, 0.95, 1.02, 100)); // band at ~1.0 = 0.5 fib of 0..2
    const ms = analyzeMarketStructure(ks)!;
    const band = ms.ranges.find((r) => r.low <= 1.0 && r.high >= 1.0)!;
    expect(band).toBeDefined();
  ...
  it("treats missing columns as unmeasured, not zero", () => {
    // point candles: no high/low - volume still lands in the close's bin
    const points: Candle[] = Array.from({ length: 20 }, (_, i) => ({ ts: t0 + (i + 1000) * DAY, close: 1 + (i % 5) * 0.01, volumeUsd: 100 }));
    const ms = analyzeMarketStructure(points)!;
    expect(ms).not.toBeNull();
    expect(ms.bins.reduce((s, b) => s + b.volume, 0)).toBeCloseTo(2000, 3);

    // candles with no volume column are excluded from the profile, not counted as zero
    const mixed = cycle().map((k, i) => (i < 5 ? { ts: k.ts, high: k.high, low: k.low, close: k.close } : k));
    const ms2 = analyzeMarketStructure(mixed)!;
    const total = mixed.reduce((s, k) => s + (k.volumeUsd ?? 0), 0);
    expect(ms2.bins.reduce((s, b) => s + b.volume, 0)).toBeCloseTo(total, 3);
  });
});
