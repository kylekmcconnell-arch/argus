...
**Three mis-reads from the first production rescan of $HADES, fixed 2026-09-25.** (1) *Venue not identified*: Robinhood Blockscout is Cloudflare-fronted and 429s under load, and the venue read had no other source. Pons v2 tokens answer for themselves - `launchFactory()` (0x536dac9b) and `deployer()` (0xd5f39488) - so `api/launch` now falls back to two RPC calls. (2) *"52% of supply sits in 25 fresh wallets - a bundled launch"*: on EVM the bundle flag had no proof requirement at all, and the v4 PoolManager plus two lockers were counted as holders. The "fresh wallets" were EIP-7702 delegated accounts (`0xef0100…` → Simple7702Account) - Robinhood-app users. The flag now needs launch-block proof (same-block buyers with a material share, or launch-block snipers in the tape); otherwise concentration is a warning. `EVM_MARKET_CONTRACTS` in `src/lib/marketAddresses.ts` names the PoolManagers, Doppler custody and the Pons/Robinhood lockers. (3) *35 code flags*: all in the venue's shared contracts (`PonsV2MemeHook.sol`, `PonsV2LaunchFactory.sol`, …) shipped with the token's verification. `splitVenueCodeFlags` charges the token only for flags in its own contract file when the venue is known, and reports the venue's flags once, without points.

## Claims ledger: write the project's statements down, then test them (2026-09-25)

**Why.** A friend's bot summarised $HADES from the team's own narrative - "smart router across Railgun, 0xbow and Houdini, not a Houdini wrapper", "no KOLs or snipers at launch", "fees to holders like Hyperliquid" - while the client, the API and the chain said otherwise. The evidence was already in the scan; the claims had never been written down to be tested.

**How.** `api/claims` collects statements where the project makes them - the site's visible copy, the app bundle's string literals, the Telegram channel preview (title, description, member count), the token's on-chain `description()`, the aggregator blurb - and types each sentence: router, original, fees-to-holders, dev-locked, no-snipers, no-custody, compliance, audit, doxxed, partnership, with the third parties it names. `src/threat/claims.ts` then answers each against what the scan already holds: the product probe (providers the client and API actually expose), the launch trace (same-block buyers), the sell tape (launch-block snipers), the fee trace (claims and conduct), the holder table (recognised lockers). Four answers: **confirmed**, **contradicted** (the copy and the evidence disagree - a finding, +8 each, capped at 24), **unrealised** (a promise with no mechanism behind it yet - a warning), **unverifiable** (stated, never silently passed). Check row "Claims vs evidence"; panel "What the project says, and what holds".

**Backend probe.** `api/product-probe` now also reads the product's own API where the bundle names it: `/health` for the self-reported service name, and an empty POST to each quote / preview / route path - a validation error answers in the provider's error-code namespace. Hades: service `bifrost-private-api`, endpoints `/privacy/transfers/quotes` and `/privacy/quotes/preview`; a real quote returns one Houdini object with no route legs, and the chain/token catalog is Houdini's verbatim (its ObjectIds, `api.houdiniswap.com` icons, `hasCex` / `hasSelfPrivate` route flags).

**Quote-asset fee conduct.** Pons v2 pays creators in ETH or the quote token from FeeEscrow `0xd3afeb2a…`, so the in-token tracer had nothing to read. `ponsV2CreatorFeeUsage` reads the creator's internal transactions and token transfers: claims from the escrow / buyback vault, then buybacks (token received from the pool after the first claim), or ETH moved on. Hades: zero claims - the "fees to holders" model is unrealised.

**What the ledger cannot reach.** Chat history (an AMA answer, a reply in Telegram) - the router claim in the friend's summary came from there, not from any page the project publishes. When a claim is quoted from a conversation, paste it into the scan as a note and the same verdict logic applies by hand.
