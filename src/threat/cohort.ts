...
export async function buyerCohort(chain: string, address: string, verdict?: string, symbol?: string): Promise<CohortOverlap | null> {
  try {
    ...
    const r = await fetch(`/api/cohort?${q}`, { signal: AbortSignal.timeout(60000) });
    if (!r.ok) return null;
...
export async function walletTaxonomy(chain: string, address: string): Promise<WalletTaxonomy | null> {
  try {
    const r = await fetch(`/api/wallet-taxonomy?address=${encodeURIComponent(address)}&chain=${encodeURIComponent(chain)}`, { signal: AbortSignal.timeout(45000) });
    if (!r.ok) return null;
