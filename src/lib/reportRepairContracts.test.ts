...
describe("report repair contracts",()=>{
 ...
 it("does not turn passive-income discussion into a guaranteed-return finding",()=>{const r=scoreProject(analyzeContent({url:"https://example.com",status:"rendered",content:"Tools to track passive income from your investments.",title:"Example",stages:[],coverageNote:"Retrieved"}));expect(r.capApplied).not.toBe("manipulation_language");});
 ...
 it("hides an implausible circulating cap even when a receipt claimed it was measured",()=>{expect(tokenMarketPresentation({mcap:5_495_026_099_675_436,fdv:5_650_000_000,marketEvidence:{mcap:true,fdv:true,liquidityUsd:false,vol24:false,ageDays:false}} as TokenDossier)).toEqual({marketCap:null,fullyDilutedValuation:5_650_000_000,liquidityUsd:null,volume24h:null,ageDays:null});});
