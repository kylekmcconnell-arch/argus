...
export type PersonLens = "founder" | "engineer" | "investor" | "executive" | "employee";
...
export function personLenses(dossier: Dossier): PersonLens[] {
  const roleText = (dossier.basicFacts ?? []).filter(f => sourced(f) && ["current_role", "executive", "founder"].includes(f.predicate))
