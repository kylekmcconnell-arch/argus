...
export function reconReportText(
  ...
  return [
    `${recon.title || host} · ${verdict ? `${verdict.verdict} ${verdict.score ?? "N/A"}/100` : recon.retrieval.status} · website${verdict?.capApplied ? ` (score limit: ${verdict.capApplied.replace(/_/g, " ")})` : ""}`,
    recon.identityLine,
    "",
