...
describe('Report evidence accuracy regressions',()=>{
 ...
 it('does not upgrade support counts to verified facts',()=>{const html=renderToStaticMarkup(<KyleIntelligenceDecisionCanvas {...props} successful={5} nextSteps={[{label:'Verify identity'}]} composition={[{axis:'F1',label:'Identity',score:3,weight:20,rationale:'Self-claimed name only',supportCount:1}]}/>);expect(html).not.toContain('strongest verified');expect(html).toContain('Identity has recorded supporting evidence');});
