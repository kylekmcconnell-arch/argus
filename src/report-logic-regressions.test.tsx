...
describe('Report evidence accuracy regressions',()=>{
 ...
 it('does not upgrade support counts to verified facts',()=>{const html=renderToStaticMarkup(<KyleIntelligenceDecisionCanvas {...props} successful={5} nextSteps={[{label:'Verify identity'}]} composition={[{axis:'F1',label:'Identity',score:3,weight:20,rationale:'Self-claimed name only',supportCount:1}]}/>);expect(html).not.toContain('strongest verified');expect(html).toContain('Identity is documented, with 1 saved supporting source');});
 it('keeps the open-work pointer out of the verdict headline',()=>{const html=renderToStaticMarkup(<KyleIntelligenceDecisionCanvas {...props} successful={5} nextSteps={[{label:'Audit the founder identity'}]}/>);expect(html).not.toContain('Next to check:');expect(html).toContain('Audit the founder identity');});
