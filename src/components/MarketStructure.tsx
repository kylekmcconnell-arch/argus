...
import { fetchOhlcv, type Candle, type OhlcvHistory } from "../lib/priceHistory";
...
const dateLabel = (t: number, timeframe: "day" | "hour") => {
...
function Chart({ candles, timeframe, ms }: { candles: Candle[]; timeframe: "day" | "hour"; ms: MarketStructure }) {
  ...
  const line = candles.map((k, i) => `${x(i).toFixed(1)},${yP(k.c).toFixed(1)}`).join(" ");
  ...
  const prevC = hover != null && hover > 0 ? candles[hover - 1].c : null;
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {/* volume pane */}
        {candles.map((k, i) => (
          <rect key={i} x={x(i) - barW / 2} y={yV(k.v)} width={barW} height={VOL_TOP + VOL_H - yV(k.v)} fill={k.c >= k.o ? "var(--color-pass)" : "var(--color-avoid)"} opacity={0.5} />
        ))}
        ...
        {ticks.map((i) => (
          <text key={i} x={x(i)} y={H - 2} fontSize={8.5} fill="var(--color-ink-faint)" textAnchor={i === 0 ? "start" : i === n - 1 ? "end" : "middle"} className="mono">
            {dateLabel(candles[i].t, timeframe)}
          </text>
        ))}
        ...
        {hover != null && (
          <g pointerEvents="none">
            ...
            <circle cx={x(hover)} cy={yP(candles[hover].c)} r={2.5} fill="var(--color-signal)" />
          </g>
        )}
      </svg>
      ...
      {hk && hover != null && (
        ...
        >
          <div className="text-ink">{dateLabel(hk.t, timeframe)}</div>
          <div>close {px(hk.c)}{prevC != null && prevC > 0 ? <span style={{ color: hk.c >= prevC ? "var(--color-pass)" : "var(--color-avoid)" }}> {hk.c >= prevC ? "+" : ""}{(((hk.c - prevC) / prevC) * 100).toFixed(1)}%</span> : null}</div>
          <div className="text-ink-faint">range {px(hk.l)} - {px(hk.h)}</div>
          <div className="text-ink-faint">vol {money(hk.v)}</div>
        </div>
      )}
    </div>
  );
...
type PanelEntry = { hist: OhlcvHistory; ms: MarketStructure } | "thin" | "none";
...
export function MarketStructurePanel({ address, chain, pairAddress }: { address: string; chain: string; pairAddress?: string }) {
  ...
  const [view, setView] = useState<{ hist: OhlcvHistory; ms: MarketStructure } | null>(null);
