...
export function OnChainForensics({ token, onAudit }: { token: TokenDossier; onAudit: (h: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {token.deployer && <OperatorNetwork deployer={token.deployer} chain={token.chain} label={`$${token.symbol}`} onAudit={onAudit} />}
      {/* EVM deployer trail — who deployed it, who funded the gas, serial launcher? */}
      {isEvm && <EvmDeployer address={token.address} chain={token.chain} symbol={token.symbol} knownDeployer={token.deployer} />}
      {/* EVM bytecode fingerprint — rug-enabling code + byte-identical known-rug clone check */}
      {isEvm && <BytecodeForensics address={token.address} chain={token.chain} symbol={token.symbol} />}
