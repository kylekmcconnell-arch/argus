...
export function ExportMenu({ dossier }: { dossier: Dossier }) {
  ...
  const pdf = () => {
    setOpen(false);
    // The canonical print path (src/lib/printPdf.ts): the on-screen report with
    // the print stylesheet, one filename convention everywhere.
    printReportPdf(dossier.handle);
  };
  const doc = () => {
    setOpen(false);
