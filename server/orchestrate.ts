...
import { orientSubjectWithGrok, orientationHandleBound, orientationMentionLeads, projectOrientationBound } from "./subjectOrientation";
...
export async function coldIntake(ctx: CollectContext, profileAlreadyResolved = false) {
  ...
  for (const t of teamCandidates) {
    ...
    const n = norm(t.name);
    if (!h && !n) continue;
    // Never list the audited subject as founder/cofounder of itself.
    if (h && h === norm(ctx.handle)) continue;
...
export function providerBackedRoles(evidence: CollectedEvidence): SubjectClass[] {
  ...
  if (evidence.profile.profile_collection_state === "resolved" && selfDescription) {
    ...
    profileRoles.forEach((role) => {
      ...
      // It must not put FOUNDER on a PROJECT-bound brand handle.
  ...
  // never also the founder person. FOUNDER routing stays when kind is FOUNDER.
  // Other bio-classified methodologies (KOL / INVESTOR / AGENCY) still govern
  // so a bound orientation cannot overwrite them; FOUNDER never does.
  if (projectBound) {
