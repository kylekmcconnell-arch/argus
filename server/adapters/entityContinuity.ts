...
function dateFromSourceUrl(value: string): string | null {
  const match = value.match(/(?:^|\D)(20\d{2})(0[1-9]|1[0-2])([0-2]\d|3[01])(?:\D|$)/);
  return match ? `${match[1]}-${match[2]}-${match[3]}` : null;
}

function ratioFromLifecycleText(value: string): string | null {
...
function recomputeContinuityCoverage(snapshot: EntityContinuitySnapshot): EntityContinuitySnapshot {
  snapshot.events = snapshot.events.map((event) => ({
    ...event,
    date: event.date ?? event.sourceUrls.map(dateFromSourceUrl).find((value): value is string => Boolean(value)) ?? null,
  }));
  snapshot.migrationRatio ??= snapshot.events
    .map((event) => ratioFromLifecycleText(`${event.title} ${event.detail}`))
    .find((value): value is string => Boolean(value)) ?? null;
  snapshot.migrationDate ??= snapshot.events
    .filter((event) => event.kind === "token_migration" || /swap|migration/i.test(`${event.title} ${event.detail}`))
